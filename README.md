...
3. Search for a prompt mode, use `Up` / `Down` to highlight it, then press `Tab` to commit the mode.
...
- `Up` / `Down` in the mode launcher: move the highlight through the filtered prompt modes.
