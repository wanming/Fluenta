...
- Restores your clipboard after insertion. Automatic Selection Actions bind each read to the captured source app, use Accessibility first, and can then use the configured Force Selection copy fallback. A Force Selection clipboard transaction restores the previous clipboard only while Inklet still owns the temporary copy, so a later external clipboard change is not overwritten. Pressing `Command+C` twice quickly can explicitly trigger Selection Actions from the copied clipboard text.
...
- Selection Actions capture the source app and selection location, validate that source before reading, and use Accessibility to read its current selection. If Accessibility does not return selected text, the configured Force Selection mode can briefly invoke menu Copy and/or `Command+C` and read the resulting clipboard text through the protected transaction described above. You can turn Force Selection off in Settings. This path does not send browser-targeted Apple Events and does not require browser Automation permission. Pressing `Command+C` twice quickly after selecting text explicitly reads the copied clipboard text. Inklet does not save merely selected text unless a successful action is recorded in local History.
