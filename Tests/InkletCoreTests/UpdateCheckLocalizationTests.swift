...
final class UpdateCheckLocalizationTests: XCTestCase {
    ...
    private let expectedValues = [
        "en": ["Check for Updates…", "Checking for Updates…", "An Inklet update is available", "Latest: %@ (build %@)", "Current: %@", "View the release on GitHub for details.", "View on GitHub", "Later", "Inklet is up to date", "You’re using %@.", "Couldn’t check for updates", "Try again in a moment.", "Retry", "Cancel", "OK"],
        "zhHans": ["检查更新…", "正在检查更新…", "Inklet 有可用更新", "最新版本：%@（构建 %@）", "当前版本：%@", "请前往 GitHub 查看发布详情。", "在 GitHub 上查看", "稍后", "Inklet 已是最新版本", "你正在使用 %@。", "无法检查更新", "请稍后重试。", "重试", "取消", "好"],
        "zhHant": ["檢查更新…", "正在檢查更新…", "Inklet 有可用更新", "最新版本：%@（組建 %@）", "目前版本：%@", "請前往 GitHub 查看發布詳情。", "在 GitHub 上查看", "稍後", "Inklet 已是最新版本", "你正在使用 %@。", "無法檢查更新", "請稍後再試。", "重試", "取消", "好"],
        "ja": ["アップデートを確認…", "アップデートを確認中…", "Inklet のアップデートがあります", "最新: %@（ビルド %@）", "現在: %@", "詳細はGitHubのリリースをご覧ください。", "GitHubで表示", "後で", "Inkletは最新です", "%@ を使用しています。", "アップデートを確認できませんでした", "しばらくしてから、もう一度お試しください。", "再試行", "キャンセル", "OK"],
        "ko": ["업데이트 확인…", "업데이트 확인 중…", "Inklet 업데이트가 있습니다", "최신: %@ (빌드 %@)", "현재: %@", "자세한 내용은 GitHub 릴리스를 확인하세요.", "GitHub에서 보기", "나중에", "Inklet이 최신 버전입니다", "현재 %@ 버전을 사용하고 있습니다.", "업데이트를 확인할 수 없습니다", "잠시 후 다시 시도하세요.", "재시도", "취소", "확인"],
        "es": ["Buscar actualizaciones…", "Buscando actualizaciones…", "Hay una actualización de Inklet disponible", "Última: %@ (compilación %@)", "Actual: %@", "Consulta la versión en GitHub para obtener más detalles.", "Ver en GitHub", "Más tarde", "Inklet está actualizado", "Estás usando %@.", "No se pudieron buscar actualizaciones", "Inténtalo de nuevo dentro de unos instantes.", "Reintentar", "Cancelar", "Aceptar"],
        "fr": ["Rechercher des mises à jour…", "Recherche des mises à jour…", "Une mise à jour d’Inklet est disponible", "Dernière version : %@ (build %@)", "Actuelle : %@", "Consultez la publication sur GitHub pour plus de détails.", "Voir sur GitHub", "Plus tard", "Inklet est à jour", "Vous utilisez %@.", "Impossible de rechercher les mises à jour", "Réessayez dans quelques instants.", "Réessayer", "Annuler", "OK"],
        "de": ["Nach Updates suchen…", "Updates werden gesucht…", "Ein Inklet-Update ist verfügbar", "Neueste: %@ (Build %@)", "Aktuell: %@", "Details finden Sie auf GitHub.", "Auf GitHub anzeigen", "Später", "Inklet ist auf dem neuesten Stand", "Sie verwenden %@.", "Updates konnten nicht überprüft werden", "Versuchen Sie es in Kürze erneut.", "Erneut versuchen", "Abbrechen", "OK"],
        "pt": ["Buscar atualizações…", "Buscando atualizações…", "Há uma atualização do Inklet disponível", "Mais recente: %@ (compilação %@)", "Atual: %@", "Veja a versão no GitHub para obter detalhes.", "Ver no GitHub", "Mais tarde", "Inklet está atualizado", "Você está usando %@.", "Não foi possível buscar atualizações", "Tente novamente em instantes.", "Tentar novamente", "Cancelar", "OK"],
        "it": ["Cerca aggiornamenti…", "Ricerca aggiornamenti…", "È disponibile un aggiornamento di Inklet", "Più recente: %@ (build %@)", "Attuale: %@", "Visualizza la release su GitHub per i dettagli.", "Visualizza su GitHub", "Più tardi", "Inklet è aggiornato", "Stai usando %@.", "Impossibile cercare aggiornamenti", "Riprova tra poco.", "Riprova", "Annulla", "OK"],
    ]
    ...
    func testUpdateCheckPlaceholderSignaturesAreIdenticalAndCorrect() throws {
        ...
        let expectedSignatures = Dictionary(uniqueKeysWithValues: updateCheckKeys.map { key in
            ...
            switch key {
            case "update.available.latestVersion": signature = ["%@", "%@"]
    ...
    func testUpdateCheckCopyResolvesWithoutFallbackForEveryInterfaceLanguage() throws {
        let savedArgumentDomain = UserDefaults.standard.volatileDomain(forName: UserDefaults.argumentDomain)
        defer {
        ...
        for tableID in approvedTableIDs {
            ...
            argumentDomain[InkletPreferenceKeys.interfaceLanguage] = try XCTUnwrap(languages[tableID]).rawValue
    ...
    func testLatestVersionFormatsLargeBuildNumbersInEveryLanguage() throws {
        let savedArgumentDomain = UserDefaults.standard.volatileDomain(forName: UserDefaults.argumentDomain)
        defer {
            UserDefaults.standard.setVolatileDomain(savedArgumentDomain, forName: UserDefaults.argumentDomain)
            NotificationCenter.default.post(name: .inkletLanguageDidChange, object: nil)
        ...
        let languages: [String: InterfaceLanguage] = [
            "en": .english, "zhHans": .simplifiedChinese, "zhHant": .traditionalChinese,
        ...
        for tableID in approvedTableIDs {
            ...
            for buildNumber in [2_147_483_648, Int.max] {
                let buildText = String(buildNumber)
                let formatted = L10n.format("update.available.latestVersion", "2.0.0", buildText)
                XCTAssertTrue(formatted.contains(buildText), "Large build was not preserved in \(tableID)")
                XCTAssertFalse(formatted.contains("-\(buildText)"), "Large build was truncated in \(tableID)")
            }
        }
    }
