...
- Run `swift test --filter GitHubReleaseUpdateCheckerTests`. These automated fixtures cover malformed release tags and URLs, draft and prerelease responses, missing and non-uploaded `Inklet.dmg` assets, a stable response with an uploaded `Inklet.dmg`, and empty and long release notes. Production and Inklet Local have no live-app fixture switch, so this command verifies checker behavior but does not count as manual alert verification.
...
- If no genuinely newer stable release is live, the newer-release alert, **View on GitHub**, **Later**, deferred automatic alert, and update-available localization interactions remain unverified, not passed. Record that limitation explicitly; automated fixtures are not a substitute for those live-app checks.
